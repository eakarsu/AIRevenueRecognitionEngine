// AI Contract amendment impact analysis
// Auto-analyze recognition delta from contract modifications
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIRevenueRecognitionEngine'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIRevenueRecognitionEngine. Focus area: Contract amendment impact analysis. ${`Auto-analyze recognition delta from contract modifications`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Contract amendment impact analysis.\n${`Auto-analyze recognition delta from contract modifications`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'amendment-impact', context, parsed);
    res.json({ feature: 'amendment-impact', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[amendment-impact]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['amendment-impact']);
